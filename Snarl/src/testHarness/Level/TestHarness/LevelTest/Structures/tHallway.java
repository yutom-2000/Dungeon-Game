 ...
 * "b"
 */
public class tHallway {

    Posn from;
    Posn to;
    Posn[] waypoints;

    public tHallway(Posn from, Posn to) {
    ...
    public tHallway(Posn from, Posn to, Posn[] waypoints) {
        if (from.equals(to)) {
    ...
    /**
     * Generate the Pons[] along the line the posns are in axis of
     * DO NOT include either previous or next posn in the returned list of posns
     * @param previous
     * @param next
    ...
    public Posn[] getLineSegment(Posn previous, Posn next) {
        if (previous.getRow() == next.getRow()) {
            return generateRowLine(Math.min(previous.getCol(), next.getCol()), Math.max(previous.getCol(), next.getCol()), previous.getRow());
        }
        else if (previous.getCol() == next.getCol()) {
            return generateColLine(Math.min(previous.getRow(), next.getRow()), Math.max(previous.getRow(), next.getRow()), previous.getCol());
        }
        else throw new IllegalArgumentException("Must be straight line segment");
    }
    ...
    public boolean equals(Object o) {
        if (o instanceof tHallway) {
            tHallway other = (tHallway) o;
            return this.from.equals(other.from) && this.to.equals(other.to);
            //todo write something to check if the list of waypoints is equal. Shouldn't matter for now
        } else return false;
    }
}